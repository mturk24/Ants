...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(insect) == BodyguardAnt and insect.ant != None: #Tests to see if ant is  a bodyguard
                self.ant = insect.ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        counting_index = 0
        while position != hive:
            if len(position.bees) > 0 and counting_index <= self.max_range and counting_index >= self.min_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        fireant_armor = self.armor - amount
        if fireant_armor <= 0:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        self.imposter = False
        QueenAnt.double_ants.append(self)
    ...
    def action(self, colony):
        ...
        def double_damage(loc):
            if loc.name == "AntQueen":
                return
            if loc.entrance != None and loc.entrance not in visited_places:
                visited_places.append(loc.entrance)
                double_damage(loc.entrance)
            if loc.exit != None and loc.exit not in visited_places:
                visited_places.append(loc.exit)
                double_damage(loc.exit)
            if loc.ant != None:
                ant = loc.ant
                if ant != None and not (ant in self.double_ants):
                    ant.damage *= 2
                    self.double_ants.append(ant)
                if type(ant) == BodyguardAnt and ant.ant != None:
                    ant = ant.ant
        ...
        if self.imposter != True:
            ...
            return 
        visited_places = []
        visited_places.append(self.place)
